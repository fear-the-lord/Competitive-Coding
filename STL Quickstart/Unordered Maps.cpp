#include <iostream>
#include <unordered_map> //header file required for implementing unordered maps
#include <string>
using namespace std;

int main() {
    //all the functions take O(1) time on average to execute.
    //If after N elements, the table needs to be rehashed, then it would take O(N) time. 
    //everything remains the same as map. Just the underlying implementation changes
    //and it does not effect the function calls that we make. 
    unordered_map <string, int> m; //specify the datatype for <key, value>
    ...
    //we will see that all the (key, value) pairs are not sorted according to the key values as in maps.
